...
$(document).ready(function(){
	...
	$("#helpDialog").fadeTo('slow','0.85');
});

function closeHelp(){
	$("#helpDialog").slideToggle();
}
...
$(window).load(function(e) {
    getURLplace(window.location.href);
});

function getURLplace(url){
	place = url.toString();
	pl = place.split("#");
	if(pl[1] != null){
		urlPar = true;
		locName = unescape(pl[1]);
		document.getElementById('search').value = locName;
		var t = setTimeout("findSiteSearch()",1000);
	}
}
